...
export default {
  ...
  updated() {
      ...
      var regexPhone = /^([1-9]{1}[1-9]{1})\s([8-9]{1}[0-9]{3,4})[\/\-]([0-9]{4})$/;
      var regexEmail = /^([a-z]{5,})[@]([a-z]{3,}[.])([a-z]{2,3})[.]{0,1}([a-z]{0,3})$/
      var regexBirth =/^([0-9]{2})[\/\-]([0-9]{2})[\/\-]([0-9]{4})$/;
      var dateParts = this.birth.split("/");
      var birthLen = this.birth.length;
      var phoneLen = this.phone.length;
      if (phoneLen === 2) {
        this.phone += ' ';
      }
      if (phoneLen === 8) {
